...
export default function BiddingApproval() {
    const { biddings, pagination, loading, search, setSearch, status, setStatus, limit, setLimit, page, setPage, refresh, submitApproval } = useBidding('Bidding', 'Ongoing');
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow style={{ backgroundColor: '#f8fafc' }}>
                                        ...
                                        <TableHead style={thStyle}>Dibuat Oleh</TableHead>
                                ...
                                <TableBody>
                                    {biddings.map((b, i) => (
                                        <TableRow key={b.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                            ...
                                            <TableCell style={tdStyle}>{b.maker_name ?? '-'}</TableCell>
                    ...
                    <div style={{ backgroundColor: '#fff', borderRadius: '16px', width: '100%', maxWidth: '480px', padding: '24px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
                        ...
                        <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
                            {modal.action === 'approve'
                                ? `Konfirmasi persetujuan bidding ${modal.company}. Dokumen akan dilanjutkan ke tahap berikutnya.`
                                : `Konfirmasi penolakan bidding ${modal.company}. Dokumen akan dikembalikan ke status Draft.`}
                        </p>
