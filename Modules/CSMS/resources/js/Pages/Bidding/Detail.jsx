...
const STATUS_CONFIG = {
    ...
    'Obsolete':          { text: 'OBSOLETE',           color: '#78716c',        bg: 'rgba(120,113,108,0.1)' },
};
...
export default function BiddingDetail() {
    ...
    const backLink = bidding.criteria === 'PostBidding' ? '/csms/post-bidding/lists'
                   : bidding.criteria === 'Renewal'     ? '/csms/renewal/lists'
                   : '/csms/bidding/lists';
    const backText = bidding.criteria === 'PostBidding' ? 'Kembali ke Post Bidding'
                   : bidding.criteria === 'Renewal'     ? 'Kembali ke Renewal'
                   : 'Kembali ke Bidding';

    return (
        <div style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh', padding: '40px 20px' }}>
            <Head title={`Detail: ${bidding.company_name ?? ''}`} />
