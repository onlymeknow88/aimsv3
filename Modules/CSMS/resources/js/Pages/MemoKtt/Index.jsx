...
export default function MemoKttIndex() {
    ...
    const [createForm, setCreateForm] = useState({ memo_number: '', status: 'Active' });
    const [creating, setCreating]     = useState(false);

    const doFetch = useCallback(() => {
        setLoading(true);
        const params = new URLSearchParams({ search, limit, page });
        fetch(`/api/csms/memo-ktts?${params}`)
            .then(r => r.json())
            .then(d => {
                setMemos(d?.data?.data ?? []);
        ...
        <CSMSLayout>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', gap: '16px', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: 1, maxWidth: '320px' }}>
                    ...
                    <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Cari no. memo..."
            ...
            <div style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
                    ...
                    {loading ? (
                        ...
                        <div style={{ padding: '60px 40px', textAlign: 'center' }}>
                            ...
                            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>Belum ada data Memo KTT.</p>
                        </div>
                    ) : (
                                ...
                                <TableBody>
                                    {memos.map((m, i) => (
                                        <TableRow key={m.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                            ...
                                            <TableCell style={tdStyle}>{m.created_at ? new Date(m.created_at).toLocaleDateString('id-ID') : '-'}</TableCell>
                                            <TableCell style={tdStyle}>{m.files_count ?? 0} file</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
                ...
                <div style={modalOverlay}>
                    <div style={modalBox}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', padding: '12px 20px', borderTop: '1px solid var(--border-color)' }}>
                            <button onClick={() => { setShowCreate(false); setCreateForm({ memo_number: '', status: 'Active' }); }}
                            ...
                            <button disabled={creating || !createForm.memo_number.trim()}
                                onClick={() => {
                                    setCreating(true);
                                    fetch('/api/csms/memo-ktts', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]')?.content },
                                        body: JSON.stringify(createForm),
                                    }).then(() => { setShowCreate(false); setCreateForm({ memo_number: '', status: 'Active' }); doFetch(); })
