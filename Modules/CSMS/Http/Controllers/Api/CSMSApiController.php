...
class CSMSApiController extends Controller
{
    ...
    public function dashboardStats(Request $request)
    {
        ...
        $valid = DB::table('biddings')
            ->whereRaw("YEAR(created_at) IN ({$safeYears})")
            ->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])
            ->where('status', self::STATUS_APPROVED)
            ->count();
        $expires = DB::table('biddings')
            ->whereRaw("YEAR(created_at) IN ({$safeYears})")
        ...
        $donutPJO = ['name' => 'Status Sertifikat', 'actual' => $validPercent, 'target' => $expiredPct];

        // Monthly series
        $evaluatedPJO = $approvedKTT = $postBiddingChart = $renewalChart = [];
        $biddingValidChart = $riskLevelChart = $picaCountChart = [];
        $contractorClassChart = $spvStatsChart = [];
        ...
        foreach ($arrayYear as $yr) {
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                $pjoEval    = DB::table('csms_pjos')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('status', 'On Review KTT')->where('requested', 'Requested KTT')->count();
                $pjoNotEval = DB::table('csms_pjos')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('status', 'On Review Evaluator')->where('requested', 'Requested Evaluator')->count();
                $evaluatedPJO[] = ['month' => $mn, 'label' => 'Evaluated', 'label2' => 'Not Evaluated', 'count' => $pjoEval, 'count2' => $pjoNotEval];

                $kttApproved    = DB::table('csms_pjos')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('status', self::STATUS_APPROVED)->where('requested', self::STATUS_APPROVED)->count();
                $kttNotApproved = DB::table('csms_pjos')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('status', 'On Review KTT')->where('requested', 'Requested KTT')->count();
                $approvedKTT[] = ['month' => $mn, 'label' => 'Approved', 'label2' => 'Not Approved', 'count' => $kttApproved, 'count2' => $kttNotApproved];

                $pbApproved = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('criteria', self::CRITERIA_POST_BIDDING)->where('status', self::STATUS_APPROVED)->where('requested', self::STATUS_APPROVED)->count();
                $pbOngoing  = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('criteria', self::CRITERIA_POST_BIDDING)->whereIn('requested', ['Requested OHS', 'Requested D/H OHS', 'Requested Evaluator'])->count();
                $postBiddingChart[] = ['month' => $mn, 'label' => 'Approved', 'label2' => 'Not Approved', 'count' => $pbApproved, 'count2' => $pbOngoing];

                $rnApproved = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('criteria', self::CRITERIA_RENEWAL)->where('status', self::STATUS_APPROVED)->where('requested', self::STATUS_APPROVED)->count();
                $rnOngoing  = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('criteria', self::CRITERIA_RENEWAL)->whereIn('requested', ['Requested OHS', 'Requested D/H OHS', 'Requested Evaluator'])->count();
                $renewalChart[] = ['month' => $mn, 'label' => 'Approved', 'label2' => 'Not Approved', 'count' => $rnApproved, 'count2' => $rnOngoing];

                $permitValid   = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('status', self::STATUS_APPROVED)->count();
                $permitExpired = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL, self::CRITERIA_INACTIVE])->where('status', self::STATUS_INACTIVE)->count();
                $biddingValidChart[] = ['month' => $mn, 'label' => 'Izin Valid', 'label2' => 'Izin Expired', 'count' => $permitValid, 'count2' => $permitExpired];

                $riskLow  = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('risk_category', 'Rendah')->where('status', self::STATUS_APPROVED)->count();
                $riskMid  = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('risk_category', 'Menengah')->where('status', self::STATUS_APPROVED)->count();
                $riskHigh = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('risk_category', 'Tinggi')->where('status', self::STATUS_APPROVED)->count();
                $riskLevelChart[] = ['month' => $mn, 'label' => 'Rendah', 'label2' => 'Menengah', 'label3' => 'Tinggi', 'count' => $riskLow, 'count2' => $riskMid, 'count3' => $riskHigh];

                $picaOpen        = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('requested', self::STATUS_APPROVED)->where('status', 'Open')->count();
                $picaOutstanding = DB::table('csms_pjos')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('status', 'On Review Evaluator')->where('requested', 'Requested Evaluator')->count();
                $picaClosed      = DB::table('csms_pjos')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->where('status', self::STATUS_APPROVED)->count();
                ...
                $cMain   = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('status', self::STATUS_APPROVED)->where('requested', self::STATUS_APPROVED)->where('classification', 'Kontraktor Utama')->count();
                $cDirect = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('classification', 'Kontraktor Langsung')->count();
                $cSingle = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('status', self::STATUS_APPROVED)->where('requested', self::STATUS_APPROVED)->where('classification', 'Subkontraktor Tunggal')->count();
                $cJoint  = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('status', self::STATUS_APPROVED)->where('requested', self::STATUS_APPROVED)->where('classification', 'Kontraktor Bersama')->count();
                ...
                $spvRows = DB::table('biddings')->whereYear('created_at', $yr)->whereMonth('created_at', $i)->whereIn('criteria', [self::CRITERIA_POST_BIDDING, self::CRITERIA_RENEWAL])->where('status', self::STATUS_APPROVED)->whereNotNull('questionnaire')->pluck('questionnaire');
                $pop = 0; $pom = 0; $pou = 0;
        ...
        // Summary counts
        $totalBidding  = DB::table('biddings')->whereRaw("YEAR(created_at) IN ({$safeYears})")->where('criteria', self::CRITERIA_BIDDING)->count();
        $totalPB       = DB::table('biddings')->whereRaw("YEAR(created_at) IN ({$safeYears})")->where('criteria', self::CRITERIA_POST_BIDDING)->count();
        $totalRenewal  = DB::table('biddings')->whereRaw("YEAR(created_at) IN ({$safeYears})")->where('criteria', self::CRITERIA_RENEWAL)->count();
        $totalApproved = DB::table('biddings')->whereRaw("YEAR(created_at) IN ({$safeYears})")->where('status', self::STATUS_APPROVED)->count();
        $totalOnReview = DB::table('biddings')->whereRaw("YEAR(created_at) IN ({$safeYears})")->whereIn('status', [self::STATUS_ON_REVIEW_OHS, self::STATUS_ON_REVIEW_DHOHS, self::STATUS_ON_REVIEW_KTT])->count();
        $totalDraft    = DB::table('biddings')->whereRaw("YEAR(created_at) IN ({$safeYears})")->where('status', self::STATUS_DRAFT)->count();

        $availableYears = DB::table('biddings')
