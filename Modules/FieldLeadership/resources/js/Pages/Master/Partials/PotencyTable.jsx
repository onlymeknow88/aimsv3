...
function PotencyModal({ title, onClose, onSave, saving, item }) {
    const [form, setForm] = useState(item ? { ...item } : { code: '', name: '' });
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
            <div style={{ backgroundColor: '#fff', borderRadius: '12px', width: '100%', maxWidth: '480px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
                ...
                <div style={{ padding: '20px' }}>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                        <label style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '4px', display: 'block' }}>Nama Potensi</label>
                        <input
                            type="text"
                            value={form.name}
                            onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                            placeholder="Misal: Luka Ringan / Cedera"
                            style={{ width: '100%', padding: '9px 12px', border: '1px solid var(--border-color)', borderRadius: '6px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
                        />
                    </div>
                </div>
...
export default function PotencyTable({ data = [], loading, onSave, onDelete }) {
    ...
    const columns = useMemo(() => [
        ...
        { accessorKey: 'name', header: 'Nama Potensi' },
        {
            id: 'actions',
            header: 'Aksi',
            cell: ({ row }) => (
    ...
    return (
        <div style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border-color)' }}>
                <h3 style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Potensi Konsekuensi</h3>
            ...
            {modal && (
                <PotencyModal
                    title={modal.mode === 'create' ? 'Tambah Potensi Konsekuensi' : 'Edit Potensi Konsekuensi'}
                    item={modal.item}
