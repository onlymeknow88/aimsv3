...
const FIELDS = [
    { key: 'rkk', label: 'RKK', desc: 'Rencana Kerja K3' },
    { key: 'cmr', label: 'CMR', desc: 'Case Medical Rate' },
    { key: 'mmr', label: 'MMR', desc: 'Man-hours Medical Rate' },
    { key: 'ssr', label: 'SSR', desc: 'Severity Rate' },
    { key: 'asr', label: 'ASR', desc: 'Average Severity Rate' },
];

export default function HealthPerformanceModal({ isOpen, editId, form, setField, onSubmit, onClose, submitting, formError }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15,23,42,0.65)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: '16px' }} onClick={onClose}>
            <div onClick={e => e.stopPropagation()} style={{ backgroundColor: '#fff', borderRadius: '16px', width: '100%', maxWidth: '520px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', overflow: 'hidden', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '18px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <h3 style={{ fontSize: '15px', fontWeight: 800, color: '#0f172a', margin: 0 }}>
                            {editId ? 'Edit Health Performance' : 'Tambah Health Performance'}
                        </h3>
                    </div>
                    <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}><X size={18} /></button>
                </div>
                <form id="health-performance-form" onSubmit={onSubmit} style={{ padding: '20px 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div style={{ padding: '16px 24px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                    ...
                    <button type="submit" form="health-performance-form" disabled={submitting} style={{ padding: '9px 20px', backgroundColor: '#1d4ed8', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.6 : 1 }}>
