...
const BASE_URL = '/api/dashboard-portal/safety-performance';

const emptyForm = { month: '', aifr: '', ainfr: '', lti_fr: '', lti_sr: '' };

export default function useSafetyPerformance() {
    ...
    const openEditModal = (item) => {
        setEditId(item.id);
        setForm({ month: item.month?.substring(0, 7) ?? '', aifr: item.aifr ?? '', ainfr: item.ainfr ?? '', lti_fr: item.lti_fr ?? '', lti_sr: item.lti_sr ?? '' });
