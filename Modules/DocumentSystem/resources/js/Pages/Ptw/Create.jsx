...
export default function Create({ document = null }) {
    ...
    // Override handleSave for PTW API
    const handlePtwSave = async (statusType) => {
        setSaving(true);
        ...
        try {
            const url = isEdit 
                ? `/api/document-system/ptw/${document.id}` 
                : '/api/document-system/ptw';
            ...
            if (statusType === 'submit') {
                ...
                if (docId) {
                    await axios.post(`/api/document-system/ptw/${docId}/submit-review`);
                }
            }
    ...
    const handleConfirmSave = async () => {
        try {
            await handlePtwSave(confirmModal.type);
    ...
    return (
        <div style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh', padding: '40px 20px' }}>
            <Head title={isEdit ? "Edit PTW" : "Create New PTW"} />
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px', maxWidth: '800px', margin: '0 auto 24px auto' }}>
                <a href="/document-system/ptw" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: 'var(--primary)', fontWeight: 700, textDecoration: 'none', fontSize: '12px' }}>
                    <ArrowLeft size={16} /> Kembali ke PTW
                </a>
                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Siklus Pembuatan PTW Baru</span>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
                <div style={{ width: '100%', maxWidth: '800px', backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '32px', boxShadow: 'var(--shadow-premium)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', borderTop: '1px solid var(--border-color)', paddingTop: '24px' }}>
                        <a href="/document-system/ptw" style={{ display: 'inline-flex', alignItems: 'center', height: '40px', padding: '0 20px', border: '1px solid var(--border-color)', borderRadius: '8px', textDecoration: 'none', color: 'var(--text-secondary)', fontSize: '12px', fontWeight: 600 }}>
                            Cancel
                        </a>
