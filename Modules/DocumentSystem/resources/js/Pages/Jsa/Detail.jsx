    ...
    return (
        <div style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh', padding: '32px 20px' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', borderTop: '1px solid var(--border-color)', paddingTop: '16px', fontSize: '12px' }}>
                            {[
                                ...
                                { label: 'Revisi No.', value: (() => { const n = parseFloat(String(document.revision ?? '0')); return (isNaN(n) ? 0 : n).toFixed(1); })() },
                            ].map(row => (
            ...
            <ConfirmationModal
                ...
                type="generic"
                title="Perbarui JSA (Revisi)?"
                description="Apakah Anda yakin ingin memperbarui JSA ini? Dokumen baru versi revisi akan dibuat sebagai Draft."
                confirmText="Ya, Perbarui"
                cancelText="Batal"
                onConfirm={() => {
                    setIsConfirmRenewOpen(false);
                    window.location.href = `/document-system/jsa/edit/${document.id}`;
                }}
