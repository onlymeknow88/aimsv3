...
export default function Detail({ id }) {
    ...
    if (loadingData || !document) {
        return (
            <div style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh', padding: '40px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Loading JSA details...</p>
            </div>
        );
    }
    ...
    const getStatusColors = (status) => {
        const s = status ? String(status) : '1';
        switch (s) {
            case '1': return { bg: 'rgba(99, 102, 241, 0.1)', text: '#6366F1', name: 'DRAFT' };
            case '5': return { bg: 'rgba(16, 185, 129, 0.1)', text: '#10B981', name: 'ACTIVE' };
            default: return { bg: 'rgba(99, 102, 241, 0.1)', text: '#6366F1', name: 'DRAFT' };
        }
    };
            ...
            <div style={{
                ...
                <main style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', borderTop: '1px solid var(--border-color)', paddingTop: '16px', fontSize: '12px' }}>
                            <div style={{ display: 'flex', borderBottom: '1px solid #f8fafc', paddingBottom: '8px' }}>
                                ...
                                <span style={{ fontWeight: 700, color: 'var(--primary)' }}>{document.document_number || '-'}</span>
                            </div>
                            <div style={{ display: 'flex', borderBottom: '1px solid #f8fafc', paddingBottom: '8px' }}>
                                <span style={{ width: '150px', fontWeight: 600, color: 'var(--text-secondary)' }}>Detail Location</span>
                                <span>{document.detail_location || '-'}</span>
                            </div>
                            <div style={{ display: 'flex', paddingBottom: '8px' }}>
                                <span style={{ width: '150px', fontWeight: 600, color: 'var(--text-secondary)' }}>Revisi No.</span>
                                <span>{!document.revision || document.revision === '' || document.revision === null ? '0.0' : `${document.revision}.0`}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{
                        ...
                        <h4 style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '12px' }}>Deskripsi JSA</h4>
                        <div
