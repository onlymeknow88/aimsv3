...
// Warna per bulan — seperti aimsv2 multi-line per periode
const COLORS = ['#153B73', '#FF8C24', '#2FBF71', '#2D7FF9', '#F5A623', '#2FBF71'];
const MUTED  = '#94a3b8';
const P      = '#2563eb';

const CSS = `
    @keyframes safety-pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
    @keyframes safety-spin  { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
    .safety-widget-grid {
        display: grid;
    ...
    @media (max-width: 640px) { .safety-widget-grid { grid-template-columns: 1fr; } }
`;
...
function SkeletonBarChart() {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ width: '140px', height: '12px', borderRadius: '4px', backgroundColor: '#e2e8f0', animation: 'safety-pulse 1.8s infinite ease-in-out' }} />
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', height: '200px', padding: '0 8px' }}>
                {bars.map((h, i) => (
                    <div key={i} style={{ flex: 1, height: `${h}%`, borderRadius: '4px 4px 0 0', backgroundColor: '#e2e8f0', animation: `safety-pulse 1.8s infinite ease-in-out`, animationDelay: `${i * 0.1}s` }} />
                ))}
            </div>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} style={{ width: '40px', height: '8px', borderRadius: '4px', backgroundColor: '#e2e8f0', animation: 'safety-pulse 1.8s infinite ease-in-out' }} />
                ))}
            </div>
        </div>
    );
...
/** Summary cards — nilai terakhir per metric */
function SummaryCards({ summary = [], loading }) {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {loading
                ? Array.from({ length: 4 }).map((_, i) => <Skel key={i} h="52px" />)
                : summary.map((item, i) => (
...
/**
 * Line chart — sumbu X = nama metric (AIFR, AINFR, LTI FR, LTI SR)
...
export default function SafetyPerformanceWidget({ filters = {} }) {
    ...
    const fetchStats = useCallback(async (y) => {
        ...
        try {
            ...
            const res = await axios.get('/api/dashboard/safety-performance/stats', { params });
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                {loading && <RefreshCw size={14} style={{ color: '#94a3b8', animation: 'safety-spin 1s linear infinite' }} />}
            </div>
            ...
            {error ? (
                ...
                <div style={{ textAlign: 'center', padding: '32px', color: MUTED, fontSize: '13px' }}>Belum ada data Safety Performance.</div>
            ) : (
