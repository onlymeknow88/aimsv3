...
export default function CalendarofEvent({ coeEvents, loading }) {
    if (loading) {
        return (
            <div aria-busy="true" aria-label="Memuat calendar of event" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '24px', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '200px' }}>
                <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h2 style={{ fontSize: '14.5px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>CALENDAR OF EVENT</h2>
                        <div className="skeleton-dashboard" style={{ width: '60px', height: '14px', backgroundColor: '#e2e8f0', borderRadius: '4px' }} />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="skeleton-dashboard" style={{ display: 'flex', gap: '16px', borderBottom: i !== 3 ? '1px solid var(--border-color)' : 'none', paddingBottom: '10px', alignItems: 'center' }}>
                ...
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '12px', marginTop: '12px', display: 'flex', justifyContent: 'center' }}>
                    <div className="skeleton-dashboard" style={{ width: '120px', height: '14px', backgroundColor: '#e2e8f0', borderRadius: '4px' }} />
                </div>
            </div>
        );
    }
    ...
    return (
        <section aria-label="Calendar of Event" style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '24px', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <h2 style={{ fontSize: '14.5px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>CALENDAR OF EVENT</h2>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    {(coeEvents ?? []).length > 0 ? (
                        (coeEvents ?? []).slice(0, 5).map((evt, idx) => {
                            ...
                            const statusColor =
                                evt.status === 'Completed'  ? '#15803d' :
                                evt.status === 'Ongoing'    ? '#1d4ed8' :
                                evt.status === 'Cancelled'  ? '#b91c1c' : '#92400e';
                            const statusBg =
            ...
            <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '12px', marginTop: '12px', textAlign: 'center' }}>
                <a href="/coe/calendar" aria-label="Lihat kalender lengkap" style={{ fontSize: '12px', color: 'var(--primary)', fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px', minHeight: '44px', justifyContent: 'center', padding: '6px 12px' }}>
                    Lihat Kalender Lengkap <ArrowRight size={12} aria-hidden="true" />
                </a>
            </div>
        </section>
    );
